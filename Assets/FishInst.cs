...
public class FishInst : MonoBehaviour
{
  ...
  void Start()
  {
    for(int i=20;i<instPos-10;i+=8){
      ...
      Instantiate(enemies[id],new Vector3(posX,Random.Range(-1.89f,0f),i),enemies[id].transform.rotation);
  ...
  IEnumerator enemyInst(){
    while(true){
      ...
      if(id==0 || id==1)Instantiate(enemies[id],new Vector3(posX,Random.Range(-1.89f,0f),instPos),enemies[id].transform.rotation);
